...
- **재고 검증 최적화**: 체크아웃 단계에서 **Redis 실시간 재고(Soft Check)**를 조회하여 DB 부하를 최소화하고, 주문 생성 시 실패 확률을 낮춤
...
### 6-2. Batch API를 통한 N+1 호출 방지

- **상품 쿠폰 배치 계산**: 개별 상품마다 쿠폰 서버에 계산 API를 날리는 N+1 문제를 방지하기 위해, 모든 상품의 쿠폰 적용 정보를 리스트 형태로 묶어 한 번에 계산하는 **배치 계산 API**를
  지향합니다.
- **효과**: 네트워크 오버헤드를 줄이고 쿠폰 서버의 부하를 최소화하여 대규모 주문 시에도 안정적인 성능을 유지합니다.

### 6-3. BFF(Backend For Frontend)로서의 최적화
