...
### 6-2. 재고 실시간 로드 (Inventory Warm-up & Sync)
...
- **효과**:
  - **Zero Blurry Snapshot**: DB와 Redis의 상태가 교차하는 찰나의 정합성 오류를 원천 차단합니다.
  - **성능 유지**: 가장 부하가 큰 '주문 선점' 단계가 아닌 '체크아웃' 단계에서 이 무거운 동기화 작업을 수행함으로써 주문 성공률과 속도를 모두 확보합니다.
